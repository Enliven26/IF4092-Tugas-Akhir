...
class Parser:
    ...
    def parse_kotlin_file(self) -> node.KotlinFile:
        ...
        return node.KotlinFile(
            ...
            end_position=self.tokens.peek().position,
            shebang=shebang,
    ...
    def parse_primary_constructor(self) -> node.PrimaryConstructor:
        ...
        if token:
            ...
            position = parameters.start_position
    ...
    def parse_constructor_invocation(self) -> node.ConstructorInvocation:
        ...
        return node.ConstructorInvocation(
            start_position=invoker.start_position,
            end_position=self.tokens.peek().position,
            invoker=invoker,
            arguments=arguments,
        )
    ...
    def parse_annotated_delegation_specifier(
        ...
        return node.AnnotatedDelegationSpecifier(
            start_position=delegate.start_position,
            end_position=self.tokens.peek().position,
            annotations=annotations,
            delegate=delegate,
        )
    ...
    def parse_explicit_delegation(self) -> node.ExplicitDelegation:
        ...
        return node.ExplicitDelegation(
            start_position=interface.start_position,
            end_position=self.tokens.peek().position,
            interface=interface,
            delegate=delegate,
        )
    ...
    def parse_function_value_parameter(self) -> node.FunctionValueParameter:
        ...
        return node.FunctionValueParameter(
            start_position=parameter.start_position,
            end_position=self.tokens.peek().position,
            modifiers=modifiers,
            parameter=parameter,
            default=default,
    ...
    def parse_variable_declaration(self) -> node.VariableDeclaration:
        ...
        return node.VariableDeclaration(
            start_position=ident.start_position,
            end_position=self.tokens.peek().position,
            annotations=annotations,
            name=ident.value,
            type=var_type,
    ...
    def parse_property_declaration(
        ...
        if consumed is not None:
            ...
            declaration = node.VariableDeclaration(
                start_position=consumed.start_position,
                end_position=self.tokens.peek().position,
                annotations=tuple(),
                name=consumed.value,
                type=var_type,
            ...
            if self._try_accept("."):
                ...
                if self._would_accept(":"):
                    self._raise(
                        ...
                        verbose=False)
                declaration = node.VariableDeclaration(
                    start_position=receiver.subtype.subtype.start_position,
                    end_position=self.tokens.peek().position,
                    annotations=tuple(),
    ...
    def parse_type(self) -> node.Type:
        ...
        return node.Type(
            start_position=subtype.start_position,
            end_position=self.tokens.peek().position,
            modifiers=modifiers,
            subtype=subtype,
        )
    ...
    def parse_user_type(self) -> node.UserType:
        ...
        return node.UserType(
            start_position=types[0].start_position,
            end_position=self.tokens.peek().position,
            sequence=types,
        )
    ...
    def parse_type_projection(self) -> node.TypeProjection:
        ...
        if self._would_accept("*"):
            token = self._accept("*")
            return node.TypeProjectionStar(start_position=token.position, end_position=self.tokens.peek().position,)
        modifiers = self.parse_modifiers(VARIANCE_MODIFIERS)
        projection = self.parse_type()
        return node.TypeProjectionWithType(
            start_position=projection.start_position,
            end_position=self.tokens.peek().position,
            modifiers=modifiers,
        ...
        if self._is_accepting_declaration():
            ...
            if token_value in ("=", "+=", "-=", "*=", "/=", "%="):
                ...
                statement = node.Assignment(
                    start_position=statement.start_position,
                    end_position=self.tokens.peek().position,
    ...
    def parse_assignment(self) -> node.Assignment:
        ...
        if directly_assignable is not None:
            ...
            return node.Assignment(
                start_position=directly_assignable.start_position,
                end_position=self.tokens.peek().position,
        ...
        if assignable is not None:
            ...
            expr = self.parse_expression()
            return node.Assignment(
                start_position=assignable.start_position,
                end_position=self.tokens.peek().position,
    ...
    def parse_additive_expression(self) -> node.Expression:
        ...
        while self._would_accept_either(*operators):
            ...
            left = node.AdditiveExpression(start_position=left.start_position,
                                           end_position=self.tokens.peek().position,
                                           operator=operator,
                                           left=left,
                                           right=right)
    ...
    def parse_prefix_unary_expression(self) -> node.Expression:
        ...
        if prefixes:
            ...
            return node.PrefixUnaryExpression(
                start_position=expression.start_position,
                end_position=self.tokens.peek().position,
                prefixes=prefixes,
            ...
            with self.tokens:
                ...
                if isinstance(postfix, node.PostfixUnaryExpression):
                    ...
                    return node.DirectlyAssignableExpression(
                        start_position=postfix.start_position,
                        end_position=self.tokens.peek().position,
                        expression=postfix,
                    )
                raise ParserException
    ...
    def parse_call_suffix(self) -> node.CallSuffix:
        ...
        if generics:
            position = generics.start_position
        elif arguments is not None:
            position = arguments.start_position
        elif lambda_expression is not None:
            position = lambda_expression.start_position
        else:
    ...
    def parse_annotated_lambda(self) -> node.AnnotatedLambda:
        ...
        return node.AnnotatedLambda(
            start_position=literal.start_position,
            end_position=self.tokens.peek().position,
            annotations=annotations,
            label=label,
            value=literal,
    ...
    def parse_lambda_parameter(self) -> node.LambdaParameter:
        ...
        return node.LambdaParameter(
            start_position=variable.start_position,
            end_position=self.tokens.peek().position,
            variable=variable,
            type=param_type,
        )
    ...
    def parse_when_entry(self) -> node.WhenEntry:
        ...
        return node.WhenConditionEntry(
            start_position=conditions[0].start_position,
            end_position=self.tokens.peek().position,
    ...
    def parse_unescaped_annotation(self) -> node.UnescapedAnnotation:
        ...
        return node.UnescapedAnnotation(
            start_position=user_type.start_position,
            end_position=self.tokens.peek().position,
            name=str(user_type),
            arguments=arguments,
        )
    ...
    def _parse_ambiguous_receiver(
        ...
        def consumed_identifier(
            ...
            ident = node.SimpleIdentifier(
                ...
                start_position=ident_type.start_position,
                end_position=self.tokens.peek().position,
            )
            return ident
        ...
        def remove_receiver_name(
            ...
            user_type = node.UserType(
                start_position=user_type.start_position,
                end_position=user_type[-2].end_position,
            ...
            type_reference = node.TypeReference(
                start_position=type_reference.start_position,
                end_position=type_reference.end_position,
                subtype=user_type,
            )
            return node.ReceiverType(
                start_position=receiver.start_position,
                end_position=receiver.end_position,
