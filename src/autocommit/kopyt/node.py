...
DelegationSpecifier = Union[
    "ConstructorInvocation", "ExplicitDelegation", "UserType", "FunctionType"
]
...
class FunctionDeclaration(Declaration):
    ...
    def __str__(self) -> str:
        ...
        return (
            f"{modifiers}fun {generics}{receiver}{self.name}"
...
class PropertyDeclaration(Declaration):
    ...
    def __str__(self) -> str:
        ...
        return (
            f"{modifiers}{self.mutability!s} {generics}{receiver}"
...
class Type(Node):
    modifiers: Modifiers
    subtype: Union[ParenthesizedType, NullableType, TypeReference, FunctionType]
...
class ForStatement(LoopStatement):
    ...
    def __str__(self) -> str:
        ...
        return f"for ({annotations}{self.variable!s} in {self.container!s})" f"{body}"
...
class LambdaParameter(Node):
    ...
    def __str__(self) -> str:
        if self.type is not None and isinstance(
            self.variable, MultiVariableDeclaration
        ):
...
class MultiAnnotation(Annotation, Nodes[UnescapedAnnotation]):
    def __str__(self) -> str:
        annotation = Annotation.__str__(self)
        unescaped = " ".join(
            UnescapedAnnotation.__str__(instance) for instance in self.sequence
        )
        return f"{annotation}[{unescaped}]"
